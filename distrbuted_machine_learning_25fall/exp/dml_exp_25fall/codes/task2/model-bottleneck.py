...
def train(model, dataloader, loss_fn, optimizer, num_epochs=2, comm_method="allreduce", writer=None, bottleneck_type="none", bottleneck_rank=0, sleep_time=0.001):
    print("Device {} starts training with {} communication method and {} bottleneck ...".format(dist_utils.get_local_rank(), comm_method, bottleneck_type))
    loss_total = 0.
    model.train()
    ...
    for epoch in range(num_epochs):
        for i, batch_data in enumerate(dataloader):
            ...
            # Apply time delay bottleneck
            if bottleneck_type == "time_delay" and dist_utils.get_local_rank() == bottleneck_rank:
...
def parse_args():
    ...
    parser.add_argument('--comm_method', default='allreduce', type=str, choices=['allreduce', 'allgather', 'reduce'], 
                        help='Communication method for gradient averaging')
    parser.add_argument('--bottleneck_type', default='none', type=str, choices=['none', 'time_delay', 'uneven_batch'], 
                        help='Type of bottleneck to apply')
    parser.add_argument('--bottleneck_rank', default=0, type=int, help='Rank to apply bottleneck')
    parser.add_argument('--sleep_time', default=0.001, type=float, help='Sleep time for time delay bottleneck')
...
def main(rank, args):
    ...
    if dist_utils.get_local_rank() == 0:
        writer = SummaryWriter(log_dir=f'./runs/{args.comm_method}_{args.bottleneck_type}')
    ...
    # Apply uneven batch size bottleneck
    batch_size = 32
    if args.bottleneck_type == "uneven_batch" and dist_utils.get_local_rank() == args.bottleneck_rank:
        batch_size = 128  # Larger batch size for bottleneck rank
    elif args.bottleneck_type == "uneven_batch":
        batch_size = 16   # Smaller batch size for other ranks
    ...
    # Train with specified communication method and bottleneck
    avg_comm_time = train(model, train_loader, loss_fn, optimizer, comm_method=args.comm_method, writer=writer, 
                         bottleneck_type=args.bottleneck_type, bottleneck_rank=args.bottleneck_rank, sleep_time=args.sleep_time)
